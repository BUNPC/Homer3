    ...
    methods
        ...
        function val = fcallStrEncodedRun(obj, reg)
            ...
            if nargin>1
                iR = reg.irun;
                tmp = {...
                    reg.funcReg(iR).GetUsageStrDecorated('hmrR_Intensity2OD_Nirs'); ...
                    reg.funcReg(iR).GetUsageStrDecorated('hmrR_MotionArtifact'); ...
                    reg.funcReg(iR).GetUsageStrDecorated('hmrR_BandpassFilt_Nirs'); ...
                    reg.funcReg(iR).GetUsageStrDecorated('hmrR_OD2Conc_Nirs'); ...
                    reg.funcReg(iR).GetUsageStrDecorated('hmrR_StimRejection'); ...
                    reg.funcReg(iR).GetUsageStrDecorated('hmrR_BlockAvg_Nirs','dcAvg'); ...
                };
                for ii=1:length(tmp)
                    if ~isempty(tmp{ii})
                        v = tmp;
