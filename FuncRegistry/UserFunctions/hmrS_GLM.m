...
% stimRuns, probe, mlActRuns, AauxRuns, tIncAutoRuns, rcMapRuns
% concatenate stims
stim = stimRuns{1};
for j = 1:size(stimRuns{1},2)
    
    foo_stim_data = stimRuns{1}(j).data; % get data from the first run
    foo_stim_states = stimRuns{1}(j).states;
        for i = 1:size(dcRuns,2)-1  % concatenate data from other runs to first run
...
foo_tIncAutoRuns = tIncAutoRuns{1};
foo_mlActRuns = mlActRuns{1};
foo_AauxRuns = AauxRuns{1};
for i = 1:size(dcRuns,2)-1
    foo_tIncAutoRuns = cat(1,foo_tIncAutoRuns,tIncAutoRuns{i+1});
    foo_mlActRuns = cat(1,foo_mlActRuns,mlActRuns{i+1});
    foo_AauxRuns = cat(1,foo_AauxRuns,AauxRuns{i+1});
end

tIncAuto = foo_tIncAutoRuns;
mlActAuto =  foo_mlActRuns;
Aaux =  foo_AauxRuns;
...
for iBlk=1:length(data_y)
    ...
    if size(dcRuns,2)>1
        ...
        for r = 1:size(dcRuns,2)
            xDrift = ones(sz(r),driftOrder);
            for ii=2:(driftOrder+1)
                xDrift(:,ii) = ([1:sz(r)]').^(ii-1);
        ...
        xDrift = ones(sz(1),driftOrder);
        for ii=2:(driftOrder+1)
            xDrift(:,ii) = ([1:sz(1)]').^(ii-1);
            xDrift(:,ii) = xDrift(:,ii) / xDrift(end,ii);
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for conc=1:2 %only HbO and HbR
        ...
        for iSS = 1:length(mlSSlst)
            ...
            % concetanted final At for session: get separate regressor fore the aux for each run 
            naux = (size(At,2)-size(dA,2));
            fooAt = zeros(size(At,1),naux*size(dcRuns,2));
            
            fooAt(1:sz(1),1:naux) =  At(1:sz(1),size(dA,2)+1:end);
            
            for i = 1:size(dcRuns,2)-1
                fooAt(sz(i)+1:sz(i)+sz(i+1),naux+1:naux*(i+1)) =  At(sz(i)+1:sz(i)+sz(i+1),size(dA,2)+1:end);
            end
