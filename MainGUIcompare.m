...
%     undisable below in order to see the check box next to "tool" menu 
%     set(h, 'checked', 'on');
...
switch(guiname)
    ...
    case 'PatchCallback'
        Display(maingui.handles, maingui.handles.axesData);  % Redisplay data axes since stims might have edited  
    case 'nirsplotLoadFileGUI'
        disp('NirsPlot Called, Please Select the Data you wish to evaluate');
end
...
if  ~maingui.dataTree.currElem.IsGroup()
    MessageBox('Exporting mean HRF at this time, only applies to the currently selected group. Please select a group in the  ProcesCurrentsing Element panel. Then rerun the export')
    return 
end
